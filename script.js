...
function handleLetters(m){ // [vowels, consonants] eaCH ITEM IS An array
	...
	for(var i=0 ; i<4 ; i++){
        if(i === 0){
		    letterRow.append("<div class='col-md-1 col-md-offset-2 col-sm-1 col-sm-offset-2 centerAlignContents'><button class='btn btn-ltr btn-warning btn-lg'>"+m[1][i]+"</button></div>");
        } else {
            letterRow.append("<div class='col-md-1 col-sm-1 centerAlignContents'><button class='btn btn-ltr btn-warning btn-lg'>"+m[1][i]+"</button></div>");
        }
        if(i !== 3){
            letterRow.append("<div class='col-md-1 col-sm-1 centerAlignContents'><button class='btn btn-ltr btn-danger btn-lg'>"+m[0][i]+"</button></div>");
        }
	}
}
...
/*
	checks and prevents the user from selecting two of the same letter in a word
	letter =  the letter the user clicked
	returns boolean
*/
...
$(document).on('click', '#letterRow div button', function(){
	"use strict";
	if(inLobby){
